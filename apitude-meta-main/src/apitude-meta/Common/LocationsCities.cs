...
{
    public class LocationsCities
    {
        public async Task<List<Cities>> GetDataFromSupplier()
        {
            ...
            var ZonesResponse = await Common.GetResponse(request, "CityList", "POST");
            ...
            XElement CityListElement = xmlDoc.Root?
                .Element(soap + "Body")?
                .Element(juniper + "CityListResponse")?
                .Element(juniper + "CityListRS")?
                .Element(juniper + "CityList");

            // Accessing data from the element, for example, iterating through Zone elements
            if (CityListElement != null)
            {
                XmlSerializer serializer = new XmlSerializer(typeof(CityList));
                CityList result;

                using (System.IO.StringReader reader = new System.IO.StringReader(CityListElement.ToString().Replace("xmlns=\"http://www.juniper.es/webservice/2007/\"","")))
                {
                    result = (CityList)serializer.Deserialize(reader);
                }

                //if(result != null && result.CityList != null && result.CityList.Count > 0)
                //{
                //    List<Cities> responseData = result.Zones.Select(x => new Cities { JPDCode = x.JPDCode, ParentJPDCode = x.ParentJPDCode, AreaType = x.AreaType, Searchable = x.Searchable, Code = x.Code, ParentCode = x.ParentCode, Name = x.Name }).ToList();
